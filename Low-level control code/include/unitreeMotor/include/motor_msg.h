...
// Send with a single data data structure
typedef union{
        int32_t           L;
        uint8_t       u8[4];
       uint16_t      u16[2];
...
typedef struct {
	...
	unsigned char  motorID;      // Motor ID 0,1,2,3 ... 0xBB means broadcast to all motors (no return at this time)
...
typedef struct {
	// Definition Motor Control Command Packet
    COMHead head;    
    MasterComdV3 Mdata;
    COMData32 CRCdata;
...
//  00 00 00 00 00 
//  00 00 00
// Packets are initialized by default
...
/*
				 ...
				 Tx485Data[_FR][i].head.motorID = i;    0                                          // Target motor label
