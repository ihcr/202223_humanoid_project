...
int main(){
    ...
    motor_run.mode = 10;
    motor_run.T = 0.0;
    motor_run.W = 0.0*9.1; //has no effect
    motor_run.Pos = 0.0*9.1;
    motor_run.K_P = 0.0;
    motor_run.K_W = 0.0; //when set to 0 motor is in damping mode, i.e. it moves as easily as if it was not powered
    ...
    while(true){
    	...
    	std::cout << "Pos: " << motor_r.Pos/9.1 << std::endl;
        std::cout << "Pos: " << (motor_r.Pos/9.1)*180/3.14159265359  << std::endl;
