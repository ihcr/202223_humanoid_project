...
#define KI 0.01f // gain of the integral controller
#define KD 0.01f // gain of the derivative controller
#define frequency 500.0f // frequency of sending commands [Hz]
#define run_time 10.0f // run time of the motor [s] 
#define T_limit 0.5f  // maximum output torque limit
#define initial_position_difference 3.14/2 // value added to the starting position of the motor to get target position [rad]
...
int main(){
    ...
    auto *logData = new float[iterations][8]; //create a new array for storing measured data
...
*/
    ...
    output << "KP=" << KP << " KI=" << KI << " KD=" << KD << " f[Hz]=" << frequency << " T_limit[Nm]=" << T_limit << " Initial error[rad]=" << initial_position_difference << " " << load << ", 0, 0, 0, 0, 0, 0, 0" << std::endl;
    output <<"time[s], Position Error[rad], Target Position[rad], Measured Position[rad], Integral, Derivative, PID Output[Nm], Motor Temperature[°C]" << std::endl;

    // loop for adding all the rows to the csv
    for (int i = 0; i < iterations; i++) {
        // print data in CSV format
        output << logData[i][0] << "," << logData[i][1] << "," << logData[i][2] << "," << logData[i][3] << "," << logData[i][4] << "," << logData[i][5] << "," << logData[i][6] << "," << logData[i][7] << std::endl;
    }
    output.close();
