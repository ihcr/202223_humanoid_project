...
#
    ...
    while current_time - start_time < MAX_TIME_SECS:
        ...
        # Assign the target positional and rotational coordinates for each end effector
        target_pos = [
            [0.0,    0.0,     0.0], #left foot
            [0.0,    -0.28,    -0.6], #left hand
            [0.0,    0.0,     0.0], #right foot
            [0.0,    -0.28,    -0.6], #right hand
            [0.0,    0.20,     -0.4]  #torso
            ]
        target_ori = [
            [0, 0, 0],  #left foot
        ...
        # To calculate the trajectory for the targets, assign the input coordinates as either absolute targets with assignWorldTarget()
        # or as the change in coordinates with assignRelativeTarget()  
        traj_interval = assignRelativeTarget(planner, target_dict, target_pos, target_ori)
        # Once calculated, execute the trajectory through the whole body controller
        current_time = executeMovement(controller, planner, env, sim_robot, joint_names, current_time, target_dict, traj_interval)
        
        print("Drawing step - clamp object")
        target_pos = [
            [0.0,    0.0,     0.0], #left foot
            [-.18,    0,  0.0 ], #left hand
            [0.0,    0.0,     0.0], #right foot
            [0.18,    0,   0.0], #right hand
            [0.0,    0.0,     0.00]  #torso
            ]
        target_ori = [
            [0, 0, 0],  #left foot
        ...
        print("Drawing step - stand up 2")
        target_pos = [
            [0.0,    0.0,     0.0], #left foot
            [-0.0,    0.1,   0.6], #left hand
            [0.0 ,   0.0,     0.0], #right foot
            [0.0,    0.1,   0.6], #right hand
            [0,    0.0,     0.4]  #torso
            ]
        target_ori = [
            [0, 0, 0],  #left foot
