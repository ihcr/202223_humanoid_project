...
def assignWorldTarget(planner, target_dict, target_pos, target_ori):    
    ...
    """
    end_target_dict["target pos"][0][0] = target_pos[0]
    end_target_dict["target pos"][0][1] = target_pos[1]
    end_target_dict["target pos"][0][2] = target_pos[2]
    
    end_target_dict["target ori"][0][0] = target_ori[0]
    end_target_dict["target ori"][0][1] = target_ori[1]
    end_target_dict["target ori"][0][2] = target_ori[2]
    """
    end_target_dict["target pos"][1][0] = target_pos[0]
    end_target_dict["target pos"][1][1] = target_pos[1]
    end_target_dict["target pos"][1][2] = target_pos[2]
    
    end_target_dict["target ori"][1][0] = target_ori[0]
    end_target_dict["target ori"][1][1] = target_ori[1]
    end_target_dict["target ori"][1][2] = target_ori[2]
...
def executeMovement(controller, planner, env, sim_robot, joint_names, current_time, target_dict, traj_interval):
    ...
    for i in traj_interval:
       ...
       # print(joint_des_pos)
        ...
        """
        time.sleep(5)
        joint_des_pos[0:23] = 0
        for j in range(6):
            for k in range(100):
                joint_des_pos[j] = 0.01 * k
                #joint_des_pos[j+6] = 0.01 * k
                #joint_des_pos[j+12] = 0.01 * k
                #joint_des_pos[j+18] = 0.01 * k
                print(joint_des_pos)
                time.sleep(0.03)
