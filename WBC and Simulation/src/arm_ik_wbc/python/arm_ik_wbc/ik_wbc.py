...
class IkWBC:
    def __init__(self, urdf_path, mesh_dir_path, EE_frame_names, EE_joint_names, base_frame, init_joint_config, fixed_base=True, foot_offset=False):
        print("ik_wbc >>> Initialise Pinoccio Model")
        """ Initialise Pinocchio model, data and geometry model """
        if fixed_base:
            self.robot_model= pin.buildModelFromUrdf(urdf_path)
        else:
            #LOAD MODEL FOR FLOATING BASE. TEST WITH JUST ARM FIRST """
        ...
        if fixed_base:
            ...
            print("No fixed base")
            self.n_velocity_dimensions = self.robot_model.nv #- 6
            self.n_configuration_dimensions = self.robot_model.nq #- 1
        ...
        if self.base_frame_index == len(self.robot_data.oMf):
                self.base_frame_index = self.robot_model.getFrameId(base_frame, pin.FIXED_JOINT)
        ...
        for i in range(self.n_of_EE):
            ID = self.robot_model.getFrameId(self.EE_frame_names[i], pin.BODY)
            """
            if ID == len(self.robot_data.oMf):
    ...
    def updateState(self, joint_config):
        ...
        #ID = self.robot_model.getFrameId(self.EE_frame_names[1], pin.FIXED_JOINT)
        #print(self.robot_data.oMf[ID])
    ...
    def jointPosLimitsArray(self, initial_config=False):
        ...
        #print("jointPosLimitsArray() >>> calculating joint limits in terms of velocity") 
        #print(self.robot_model.lowerPositionLimit)
    ...
    def findAb(self, targets_dict): # target_dict = {"task name":[], "target pos":[], "target ori":[]}
        ...
        for i in range(len(self.active_task_dict["task name"])):
            ...
            if self.active_task_dict["task type"][i] == "CoM Cart":
                ...
                # The target coordinates for the CoM task should be assigned automatically depending on an assigned task
                # Previously it would be assigned to the target coordinates of one of the end effectors, but sometimes
                # the target position of this end effector (even if its the torso) needs to be different to the CoM of the robot
                # This is especially the case when disregarding the vertical axis target for CoM
                # It also would not work with relative IK task targets, instead of world targets
                A_tmp, b_tmp = self.active_task_dict["task instance"][i].CoMCartesianFindAb(self.robot_model, self.robot_data, self.current_joint_config, np.array([[-0.140000], [-0.000], [0.000]]), self.dt)
                A[append_index:(append_index+self.active_task_dict["task instance"][i].n_of_targets)] = A_tmp
                b[append_index:(append_index+self.active_task_dict["task instance"][i].n_of_targets)] = b_tmp
                append_index = append_index + self.active_task_dict["task instance"][i].n_of_targets
            if self.active_task_dict["task type"][i] == "CoM Stab":
    ...
    """ Control Modes """
    def GeneralMode(self, high_dof=False) :

        #[["left_foot", ["IK", False, False, False, False, False, False, False], "CoM"]],[]]
        ...
        for i in range(self.n_velocity_dimensions):
            ...
            task_parameters.append([i, self.n_velocity_dimensions, 0.000001, "PREV", 0, self.init_joint_config])
        ...
        # Assign the CoM task
        task_names.append("torso_centre_joint")
        task_types.append("CoM Cart")
        task_parameters.append([1, 1])
        ...
        for i in range(self.n_of_EE):
            ...
            # The right foot needs to be constrained in every axis to keep it from moving since it cannot be fixed like the left ankle, which is the base link
            if self.EE_joint_names[i] == "right_ankle-2":
                cstrnt_parameters.append([self.EE_index_list_frame[i], pin.ReferenceFrame.LOCAL_WORLD_ALIGNED, True, True, True, True, True, True, False])
            # All other end-effectors are unconstrained
            else:
